...
class MidSharedBertPreTrainedModel(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained models.
    """
    def __init__(self, config, *inputs, **kwargs):
        super(MidSharedBertPreTrainedModel, self).__init__()
        if not isinstance(config, BertConfig):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class `BertConfig`. "
                "To create a model from a Google pretrained model use "
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, ae_path, *inputs, **kwargs):
        ...
        mid_layer_pattern = []
        for i in range(1, 11):
            mid_layer_pattern.append('.'+str(i)+'.')
        for key in list(ae_state_dict.keys()):
            flag = 1
            for pattern in mid_layer_pattern:
                if pattern in key:
                    flag = 0
                    break
            if flag:
...
class CNNLayer(nn.Module):
    def __init__(self, input_size, in_channels, out_channels,
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, (kernel_width, input_size), padding=(int(kernel_width/2), 0))
...
class CategoryClassification(BertPreTrainedModel):

    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(CategoryClassification, self).__init__(config)
