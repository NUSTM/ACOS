...
def measureTriple(triple_preds, triplegolds, task_name):
    triple_preds = triple_preds[0]
    tp = .0
    fp = .0
    fn = .0
    for i in range(len(triple_preds)):
        cnt = 0
        for triple in triple_preds[i]:
            try:
                if triple in triplegolds[i]:
                    cnt += 1
            except:
                pdb.set_trace()
        tp += cnt
        fp += len(triple_preds[i])-cnt
        fn += len(triplegolds[i])-cnt
    p = 0 if tp + fp == 0 else 1.*tp / (tp + fp)
    r = 0 if tp + fn == 0 else 1.*tp / (tp + fn)
    f = 0 if p + r == 0 else 2 * p * r / (p + r)
    return {'precision':p, 'recall':r, 'macro-F1':f}
